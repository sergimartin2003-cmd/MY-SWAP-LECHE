...
export function useParaSwapQuote(
  ...
  const fetchQuote = useCallback(async () => {
    ...
    if (!amountIn || isNaN(num) || num <= 0) { setQuote(null); return; }
    ...
    try {
      ...
      const url = `${PARASWAP_BASE}/prices?srcToken=${srcToken}&destToken=${destToken}&amount=${amountWei}&srcDecimals=${srcDecimals}&destDecimals=${destDecimals}&side=SELL&network=${chainId}&partner=NexSwap`;
      const res = await fetch(url, { signal: AbortSignal.timeout(10_000) });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      const pr = data.priceRoute;
      const destFormatted = (parseInt(pr.destAmount, 10) / 10 ** destDecimals).toFixed(destDecimals > 6 ? 6 : destDecimals);
      setQuote({ srcAmount: pr.srcAmount, destAmount: pr.destAmount, destAmountFormatted: destFormatted, gasCostUSD: pr.gasCostUSD ?? '?', priceRoute: pr });
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Quote failed';
      if (!msg.includes('Failed to fetch') && !msg.includes('abort')) setError(msg);
      setQuote(null);
    } finally {
      setLoading(false);
    }
