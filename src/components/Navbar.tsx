...
export default function Navbar() {
  ...
  return (
    <motion.nav
      ...
      style={{ background: 'rgba(5, 5, 16, 0.85)', backdropFilter: 'blur(24px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
        <motion.div className="flex items-center gap-2 cursor-pointer" whileHover={{ scale: 1.02 }} onClick={() => setActiveTab('swap')}>
        ...
        <div className="flex items-center gap-3">
          {isConnected && address ? (
            <motion.button whileHover={{ scale: 1.02, boxShadow: '0 0 20px rgba(123,47,255,0.3)' }} whileTap={{ scale: 0.98 }}
              onClick={() => setShowWalletPanel(true)}
              className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium"
              style={{ background: 'rgba(123,47,255,0.15)', border: '1px solid rgba(123,47,255,0.35)', color: '#fff' }}>
              <div className="w-2 h-2 rounded-full bg-neon-green animate-pulse" />
              {balance && <span className="hidden sm:block text-xs font-mono text-white/60 mr-0.5">{parseFloat(balance.formatted).toFixed(3)} {balance.symbol}</span>}
              <span className="font-mono text-xs">{truncate(address)}</span>
              <ChevronDown size={13} className="text-white/50" />
            </motion.button>
          ) : (
            <motion.button whileHover={{ scale: 1.03, boxShadow: '0 0 30px rgba(123,47,255,0.4)' }} whileTap={{ scale: 0.97 }}
              onClick={() => setShowWalletPanel(true)}
      ...
      <div className="md:hidden flex items-center justify-around px-4 pb-2">
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            ...
            style={{ color: activeTab === tab.id ? '#00D4FF' : 'rgba(255,255,255,0.4)', borderBottom: activeTab === tab.id ? '2px solid #00D4FF' : '2px solid transparent' }}>
            {tab.label}
          </button>
        ))}
      </div>
