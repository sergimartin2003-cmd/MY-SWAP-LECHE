...
import { ArrowDownUp, Settings, ChevronDown, AlertTriangle, Info } from 'lucide-react';
...
export default function SwapCard() {
  const {
    tokenIn, tokenOut, amountIn, amountOut, slippage, gasSpeed, priceImpact,
    setTokenIn, setTokenOut, setAmountIn, flipTokens, setSlippage, setGasSpeed,
    walletConnected, connectWallet, addNotification,
  } = useStore();
  ...
  const handleSwap = () => {
    ...
    addNotification({ type: 'success', title: 'Swap Submitted', message: `Swapping ${amountIn} ${tokenIn.symbol} → ${amountOut} ${tokenOut.symbol}` });
    setAmountIn('');
  };
      ...
      <AnimatePresence>
        {showSettings && (
          <motion.div
            ...
            className="glass-card mb-3 p-4 overflow-hidden"
          >
            <h4 className="text-sm font-semibold text-white/70 mb-3">Transaction Settings</h4>
            <div className="mb-4">
              <p className="text-xs text-white/40 mb-2 flex items-center gap-1.5">
                Slippage Tolerance <Info size={11} className="text-white/30" />
              </p>
              <div className="flex gap-2">
                {SLIPPAGE_OPTIONS.map(s => (
                  <button
                    ...
                    style={{
                      background: slippage === s && !customSlippage ? 'rgba(123,47,255,0.3)' : 'rgba(255,255,255,0.05)',
                      border: slippage === s && !customSlippage ? '1px solid rgba(123,47,255,0.5)' : '1px solid rgba(255,255,255,0.08)',
                      color: slippage === s && !customSlippage ? '#fff' : 'rgba(255,255,255,0.5)',
                    }}
                  >
                    {s}%
                  </button>
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {GAS_OPTIONS.map(opt => (
                  ...
                  >
                    ...
                    <p className="text-xs text-white/40 mt-0.5">{opt.time}</p>
                    <p className="text-xs font-mono mt-0.5" style={{ color: '#00D4FF' }}>{opt.gwei}</p>
                  </button>
                ))}
              </div>
            </div>
      ...
      <div className="glass-card p-5" style={{ border: '1px solid rgba(123,47,255,0.2)' }}>
        <div className="flex items-center justify-between mb-4">
          ...
          <motion.button
            ...
            style={{
              background: showSettings ? 'rgba(123,47,255,0.2)' : 'rgba(255,255,255,0.05)',
              color: showSettings ? '#7B2FFF' : 'rgba(255,255,255,0.5)',
            }}
          >
            <Settings size={16} />
          </motion.button>
        ...
        <div className="flex justify-center my-2 relative z-10">
          <motion.button
            ...
            style={{ background: 'linear-gradient(135deg, rgba(123,47,255,0.3), rgba(0,212,255,0.2))', border: '1px solid rgba(123,47,255,0.3)' }}
          >
        ...
        {amountIn && amountOut && (
          ...
          >
            <div className="flex justify-between text-xs">
              ...
              <span className="text-white/70 font-mono">1 {tokenIn.symbol} = {exchangeRate.toFixed(6)} {tokenOut.symbol}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-white/40">Price Impact</span>
              <span className="font-semibold flex items-center gap-1 px-2 py-0.5 rounded-full text-xs"
                style={{ background: impactBg, color: impactColor }}>
                {priceImpact > 3 && <AlertTriangle size={10} />}
            ...
            <div className="flex justify-between text-xs">
              ...
              <span className="text-white/70">${(GAS_OPTIONS.find(g => g.speed === gasSpeed)!.gwei * 21000 * 0.000000001 * 3842).toFixed(2)}</span>
            </div>
          </motion.div>
        )}

        <motion.button
          whileHover={{ scale: 1.02, boxShadow: '0 0 40px rgba(123,47,255,0.4)' }}
          whileTap={{ scale: 0.98 }}
          onClick={handleSwap}
        ...
        >
          {!walletConnected
            ...
            : !amountIn
              ? 'Enter Amount'
              : priceImpact > 15
                ? '⚠️ High Price Impact — Swap Anyway'
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-2">
        ...
        {walletConnected && token.balance !== undefined && (
