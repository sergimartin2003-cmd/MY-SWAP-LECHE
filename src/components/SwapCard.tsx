...
export default function SwapCard() {
  const {
    ...
    slippage, gasSpeed, priceImpact,
    setTokenIn, setTokenOut, setAmountIn, setAmountOut, flipTokens,
    setSlippage, setGasSpeed,
    setShowWalletPanel, addNotification,
  } = useStore();
  ...
  useEffect(() => {
    if (quote) {
      setAmountOut(quote.destAmountFormatted);
    }
  }, [quote, setAmountOut]);
  ...
  useEffect(() => {
    if (swapStatus === 'success') {
      addNotification({
        ...
        message: `${amountIn} ${tokenIn.symbol} → ${displayAmountOut} ${tokenOut.symbol}`,
      });
      setAmountIn('');
      setTimeout(resetSwap, 3000);
    }
    if (swapStatus === 'error' && execError) {
      addNotification({ type: 'error', title: 'Swap failed', message: execError });
      setTimeout(resetSwap, 4000);
    }
